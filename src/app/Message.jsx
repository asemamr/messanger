...
export default function Message({ messageId, message , session}) {
  ...
  return (
    <div key={messageId} className={`mx-10 flex items-center  my-5 space-x-3 ${user && " space-x-3 text-right flex-row-reverse  justify-start"}`}>
      ...
      <div>
      <p className={`text-blue-300 text-sm font-bold ${!user && "text-red-300 pl-3"} mb-1`}>{message.username }</p>
